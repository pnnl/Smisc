...
##'
##' Linear mapping of a numerical vector or scalar from one contiguous interval to another
##'
##' The mapping is \eqn{f : D \rightarrow R}, where \eqn{f(D[1]) = R[1]} and \eqn{f(D[2]) = R[2]}.
##' 
##' @export
##' @param x a numeric vector
##' @param D a vector with 2 elements, the first being the lower endpoint of the domain,
##' the upper being the upper endpoint of the domain. Note \code{R[1]} must be less than \code{R[2]}.
##' @param R a vector with 2 elements indicating the range of the linear mapping. \code{R[1]}
##' is mapped to \code{D[1]}, and \code{R[2]} is mapped to \code{D[2]}.
...
##' \eqn{f(D[2]) = R[2]}.
##' 
##' @usage linearMap(x, D = range(x), R = c(0, 1))
##' @param x a numeric vector
